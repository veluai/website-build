# Home Page Copy

## Page Status

- Page name: Home
- Canonical URL: `https://velu.us/`
- Publication status: Not published
- Repository status before this task: Non-existent
- Visible copy: Implemented in GoHighLevel
- Desktop review: Completed
- Mobile review: Deferred
- Publication: Pending
- Live verification: Pending

## Global Header

- Velu logo linking to `https://velu.us`
- Services
  - Outsourced Accounting
  - Client Advisory Services
  - Grant Advisory & Compliance
  - Tax
- About
- Client Portal
- Get Started

Client Portal destination:

`https://clientlogin-us2.karbonhq.com/v14Bq1BSNtF/Identity/Account/Login?token=`

## Approved Visible Page Copy

## Section 1: Hero

# Financial Clarity for Nonprofits Ready to Grow

We help nonprofit leaders understand their numbers, align financial decisions with their mission, and build organizations that thrive long after they no longer need us.

Primary CTA:

`Book Your Discovery Call`

Primary CTA URL:

`https://velu.us/get-started/`

Hero microcopy:

`No obligation · 45-minute conversation · See if we're the right fit`

## Section 2: Problem

## Growing a Nonprofit Is Hard When Your Finances Are Holding You Back.

Nonprofit leaders make decisions every day that affect their mission, funding, and future. Without financial clarity, those decisions become harder than they need to be.

### Unclear Financial Picture

Financial reports may tell you what happened last month, but not what it means for your organization's future.

### Funding Feels Unpredictable

Without reliable forecasting and planning, every funding decision can feel reactive instead of strategic.

### Programs and Finances Don't Connect

Many nonprofits struggle to understand how funding, staffing, and program outcomes work together.

### Growth Creates Complexity

As organizations grow, financial processes, reporting requirements, and operational demands become harder to manage.

### Limited Financial Leadership

Not every nonprofit needs a full-time CFO, but every growing nonprofit needs financial guidance.

### Mission Gets Stuck in Administration

Your leadership team should spend more time advancing your mission and less time worrying about accounting.

## Section 3: Services

## How We Help Nonprofits Move Forward

Every nonprofit's journey is different, but sustainable growth starts with the right financial foundation, guidance, and strategy.

### Outsourced Accounting

Service label:

`Outsourced Accounting`

Outcome heading:

`Build Your Financial Foundation`

Link:

`https://velu.us/services/outsourced-accounting-services`

### Client Advisory Services

Service label:

`Client Advisory Services`

Outcome heading:

`Connect Programs and Finances`

Link:

`https://velu.us/services/client-advisory-services`

### Grant Advisory & Compliance

Service label:

`Grant Advisory & Compliance`

Outcome heading:

`Maximize Funding Opportunities`

Link:

`https://velu.us/services/grant-advisory-compliance-services`

### The Outcome

Outcome label:

`The Outcome`

Outcome heading:

`Plan for Sustainable Growth`

Supporting copy:

`Bring reliable accounting, financial insight, and funding strategy together so your organization can plan for what’s next with confidence.`

Link:

`https://velu.us/get-started/`

## Section 4: Why Nonprofits Choose Velu

## Why Nonprofits Choose Velu

We combine nonprofit expertise, strategic guidance, and a collaborative approach to help organizations build stronger financial futures.

### Nonprofit Certified Expertise

### A Team-Based Approach

### Proactive Advisory, Not Just Accounting

### Financial Partner for Every Stage of Growth

### Focused on Long-Term Self-Sufficiency

Quote:

`Our goal isn't to make you dependent on us. It's to help you build an organization that thrives without us.`

## Section 5: Reviews

## What Nonprofit Leaders Say About Working With Velu

Helping organizations build stronger financial futures is more than our goal—it's what our clients experience every day.

Reviews:

- 14 visible Google reviews
- Full review details are documented in `page-assets/home-page-review-inventory.md`

Do not duplicate all review text in this copy file. Use the review inventory as the source record for review details.

## Section 6: Final CTA

## Ready to Move From Insight to Foresight?

When leadership understands the financial story behind the organization... better decisions follow.

Benefit list:

- Understand financial performance
- Evaluate opportunities and risks
- Plan for sustainable growth
- Make informed leadership decisions

Final CTA:

`Book Your Discovery Call`

Final CTA URL:

`https://velu.us/get-started/`

Final CTA microcopy:

`No obligation · 45-minute conversation · See if we're the right fit`

## Heading Hierarchy

- H1: Financial Clarity for Nonprofits Ready to Grow
- H2: Growing a Nonprofit Is Hard When Your Finances Are Holding You Back.
- H2: How We Help Nonprofits Move Forward
- H2: Why Nonprofits Choose Velu
- H2: What Nonprofit Leaders Say About Working With Velu
- H2: Ready to Move From Insight to Foresight?

Card titles and differentiator headings should be H3s or equivalent semantic subheadings beneath their section H2s.

## Global Footer

- Copyright 2026 - Velu LLC - All Rights Reserved
- Privacy Policy
- Terms and Conditions
